...
<?php
		...
		$dominio = trim($_SESSION["dominiom"]);		
		$hotel=trim($_SESSION["hotel"]);  		  
		...
		else 
		{
				...
				if(($diasHastaJuliano - $diasDesdeJuliano)<0)				
				{
					...
					echo "<script type='text/javascript'>
					alert('No puedes cancelar reservaciones menores al dia de hoy.');
				...
				else
				{
					...
					//echo $arrEmail[1];
					if(trim($dominio) == trim($arrEmail[1]))
						{
							$query_usr="UPDATE reservacion SET status='cancelada' WHERE guid='$guid'";
							  mysql_connect("db.mayakoba.com","reservation","pNTH2MBe") or die("No se pudo conectar a la base de datos");
